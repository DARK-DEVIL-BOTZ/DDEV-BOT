...
try {
    ...
    console.log("💉 𝙲𝙾𝙽𝙽𝙴𝙲𝚃𝙴𝙳 𝚃𝙾 𝚃𝙷𝙴 𝙳𝙳𝙴𝚅 𝙳𝙰𝚃𝙰𝙱𝙰𝚂𝙴");
} catch {
                    ...
                    try {
                        ...
                        if (citel.text && !citel.isGroup) {
                            if (zx < 25) {
                                ...
                                var textuser = budy
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=179125&key=NBehLa7z6HBV2SUJ&uid=[${diffuser}]&msg=[${textuser}]`);
                            ...
                            if (zx < 20) {
                                var diffuser = citel.sender.split("@")[0];
                                let fetchk = require("node-fetch");
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=179125&key=NBehLa7z6HBV2SUJ&uid=[${diffuser}]&msg=[${citel.text}]`);
                                let json = await fetchtext.json();
                                let { cnt } = json;
				    console.log(cnt)
                                citel.reply(cnt);
 ...
 Void.ev.on('group-participants.update', async(anu) => {
                    ...
                    if (checkinfo) {
                        ...
                        if(/@pp/g.test(goodbye_messages)) {
                           let buttonMessage = {
                            ...
                            let buttonMessage = {
                                image: {url : ppUrl },
                                caption: `⬆️ 𝙿𝚁𝙾𝙼𝙾𝚃𝙴 𝙷𝙰𝚂 𝙱𝙴𝙴𝙽 𝙳𝙴𝚃𝙴𝙲𝚃𝙴𝙳 ⬆️\n\n👨‍🦱 𝙽𝙰𝙼𝙴 : @${num.split("@")[0]}\n🧿 𝚂𝚃𝙰𝚃𝚄𝚂 : 𝙵𝚁𝙾𝙼 𝙼𝙴𝙼𝙱𝙴𝚁 ◈ 𝚃𝙾 𝙰𝙳𝙼𝙸𝙽\n™️ 𝙶𝚁𝙾𝚄𝙿 : ${metadata.subject}`,
                                footer: `*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴅᴅᴇᴠ ʙᴏᴛ*`,
                                mentions: [num],
                                headerType: 4,
                            }
                            ...
                            let buttonMessage = {
                                image: { url : ppUrl },
                                caption: `⬇️ 𝙳𝙴𝙼𝙾𝚃𝙴 𝙷𝙰𝚂 𝙱𝙴𝙴𝙽 𝙳𝙴𝚃𝙴𝙲𝚃𝙴𝙳 ⬇️\n\n👨‍🦱 𝙽𝙰𝙼𝙴 : @${num.split("@")[0]}\n🧿 𝚂𝚃𝙰𝚃𝚄𝚂 : 𝙵𝙾𝚁 𝙰𝙳𝙼𝙸𝙽 ◈ 𝙼𝙴𝙼𝙱𝙴𝚁™️ 𝙶𝚁𝙾𝚄𝙿 : ${metadata.subject}`,
                                footer: `*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴅᴅᴇᴠ ʙᴏᴛ*`,
                                mentions: [num],
                                headerType: 4,
                            }
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === "connecting") {
                   console.log("🎯 𝙲𝙾𝙽𝙽𝙴𝙲𝚃𝙸𝙽𝙶 𝚃𝙾 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿 𝚆𝙴𝙱... 𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃.");
                }
                if (connection === 'open') {
                    console.log("✅ 𝙳𝙳𝙴𝚅 𝙻𝙾𝙶𝙸𝙽 𝚂𝚄𝙲𝙲𝙴𝚂𝚂𝙵𝚄𝙻!");
                    console.log("⬇️ 𝙳𝙳𝙴𝚅 𝙴𝚇𝚃𝙴𝚁𝙽𝙰𝙻 𝙿𝙻𝚄𝙶𝙸𝙽𝚂 𝙸𝙽𝚂𝚃𝙰𝙻𝙻𝙸𝙽𝙶...");
                    let axios = require('axios')
                    let check = await plugindb.find({})
                    for (let i = 0; i < check.length; i++) {
                        let AxiosData = await axios.get(check[i].url)
                        let data = AxiosData.data
                         await fs.writeFileSync(__dirname + '/../ddev-plugins/' + check[i].id + '.js', data, "utf8")
                     }
                     console.log("⬇️ 𝙳𝙳𝙴𝚅 𝙴𝚇𝚃𝙴𝚁𝙽𝙰𝙻 𝙿𝙻𝙸𝙶𝙸𝙽𝚂 𝙸𝙽𝚂𝚃𝙰𝙻𝙻𝙴𝙳!");fs.readdirSync(__dirname + "/../ddev-plugins").forEach((plugin) => {
     if (path.extname(plugin).toLowerCase() == ".js") {
         console.log(`🧰 𝙻𝙾𝙰𝙳𝙸𝙽𝙶 𝙿𝙻𝚄𝙶𝙸𝙽: ${plugin}`);
         try {
             require(__dirname + "/../ddev-plugins/" + plugin);
             console.log(`✅ 𝙿𝙻𝚄𝙶𝙸𝙽 𝙸𝙽𝚂𝚃𝙰𝙻𝙻𝙴𝙳 : ${plugin}`);
         } catch (error) {
             console.error(`❌ 𝙿𝙻𝚄𝙶𝙸𝙽 𝙸𝙽𝚂𝚃𝙰𝙻𝙻 𝙵𝙰𝙸𝙻𝙴𝙳 : ${plugin}`, error);
         }
     }
 });
...
for (let i of owner) {
    const read = Config['readmessage'] == ![] ? _0x3ac3a1(0xce) : Config[_0x3ac3a1(0xcf)] == undefined ? '𝚁𝙴𝙰𝙳 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 : ❌' : Config['readmessage'] === _0x3ac3a1(0xd9) ? _0x3ac3a1(0xce) : _0x3ac3a1(0xe5),
        ars = Config[_0x3ac3a1(0xfe)] == ![] ? '𝙰𝚄𝚃𝙾 𝚁𝙴𝙰𝙳 𝚂𝚃𝙰𝚃𝚄𝚂 : ❌' : Config[_0x3ac3a1(0xfe)] == undefined ? '𝙰𝚄𝚃𝙾 𝚁𝙴𝙰𝙳 𝚂𝚃𝙰𝚃𝚄𝚂 : ❌' : Config[_0x3ac3a1(0xfe)] === _0x3ac3a1(0xd9) ? _0x3ac3a1(0xca) : _0x3ac3a1(0xd7),
        dp = Config[_0x3ac3a1(0xd2)] == ![] ? _0x3ac3a1(0xf3) : Config['disablepm'] == undefined ? '𝙳𝙸𝚂𝙰𝙱𝙻𝙴 𝙿𝙼 : ❌' : Config['disablepm'] === _0x3ac3a1(0xd9) ? '𝙳𝙸𝚂𝙰𝙱𝙻𝙴 𝙿𝙼 : ❌' : _0x3ac3a1(0xdd),
        oak = Config[_0x3ac3a1(0xe4)] == ![] ? '𝙾𝙿𝙴𝙽𝙰𝙸 𝙺𝙴𝚈 : ❌' : Config[_0x3ac3a1(0xe4)] == undefined ? _0x3ac3a1(0xdf) : Config['OPENAI_API_KEY'] === _0x3ac3a1(0xd9) ? '𝙾𝙿𝙴𝙽𝙰𝙸 𝙺𝙴𝚈 : ❌' : _0x3ac3a1(0xe6),
        ...
        ar = Config['autoreaction'] == ![] ? _0x3ac3a1(0xc7) : Config[_0x3ac3a1(0xe2)] == undefined ? _0x3ac3a1(0xc7) : Config[_0x3ac3a1(0xe2)] === _0x3ac3a1(0xd9) ? '𝙰𝚄𝚃𝙾 𝚁𝙴𝙰𝙲𝚃𝙸𝙾𝙽 : ❌' : '𝙰𝚄𝚃𝙾 𝚁𝙴𝙰𝙲𝚃𝙸𝙾𝙽 : ✅';
    Void[_0x3ac3a1(0xfd)](i + _0x3ac3a1(0xde), {
        'text': _0x3ac3a1(0xda) + events[_0x3ac3a1(0xd4)][_0x3ac3a1(0xd6)] + _0x3ac3a1(0xec) + Config['WORKTYPE'] + _0x3ac3a1(0xf1) + Config['BRANCH'] + _0x3ac3a1(0xe7) + Config[_0x3ac3a1(0xe3)] + _0x3ac3a1(0xef) + prefix + '_\x0a_𝙾𝚆𝙽𝙴𝚁 : ' + process[_0x3ac3a1(0xeb)][_0x3ac3a1(0x100)] + _0x3ac3a1(0xd1) + read + '\x0a' + ars + '\x0a' + dp + '\x0a' + oak + '\x0a' + lap + '\x0a' + ar
    });
}

function _0x1228() {
    const _0x44faa2 = ['disablepm', 'stateObject', 'commands', '900dZcbVK', 'length', '𝙰𝚄𝚃𝙾 𝚁𝙴𝙰𝙳 𝚂𝚃𝙰𝚃𝚄𝚂 : ✅', '25524zIwSmd', 'false', '_*𝙳𝙳𝙴𝚅 𝙱𝙾𝚃 𝙷𝙰𝚂 𝙱𝙴𝙴𝙽 𝙰𝙲𝚃𝙸𝚅𝙰𝚃𝙴𝙳!*_\x0a_𝚃𝙾𝚃𝙰𝙻 𝙿𝙻𝚄𝙶𝙸𝙽𝚂 : ', '\\+\\+ *(?:[a-zA-Z_$][0-9a-zA-Z_$]*)', '430htWnSK', '𝙳𝙸𝚂𝙰𝙱𝙻𝙴 𝙿𝙼 : ✅', '@s.whatsapp.net', '𝙾𝙿𝙴𝙽𝙰𝙸 𝙺𝙴𝚈 : ❌', 'while (true) {}', '2901400hFxzqi', 'autoreaction', 'LANG', 'OPENAI_API_KEY', '𝚁𝙴𝙰𝙳 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 : ✅', '𝙾𝙿𝙴𝙽𝙰𝙸 𝙺𝙴𝚈 : ✅', '_\x0a_𝚃𝙷𝙴𝙼𝙴 : ', 'string', 'gger', 'init', 'env', '_\x0a_𝙼𝙾𝙳𝙴 : ', 'test', 'apply', '_\x0a_𝙿𝚁𝙴𝙵𝙸𝚇 : ', '6474776dXZzVw', '_\x0a_𝚅𝙴𝚁𝚂𝙸𝙾𝙽 : 0.0.5_\x0a_𝙱𝚁𝙰𝙽𝙲𝙷 : ', 'debu', '𝙳𝙸𝚂𝙰𝙱𝙻𝙴 𝙿𝙼 : ❌', '𝙻𝙴𝚅𝙴𝙻 𝚄𝙿 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 : ❌', '5732LEkoNV', 'levelupmessage', 'function *\\( *\\)', 'call', '𝙻𝙴𝚅𝙴𝙻 𝚄𝙿 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 : ✅', '790152XOXqJK', 'counter', 'chain', 'sendMessage', 'auto_read_status', '1000056clyKNG', 'OWNER_NAME', '24598wXVLur', '𝙰𝚄𝚃𝙾 𝚁𝙴𝙰𝙲𝚃𝙸𝙾𝙽 : ❌', '18hoAmEM', 'action', '𝙰𝚄𝚃𝙾 𝚁𝙴𝙰𝙳 𝚂𝚃𝚂𝚃𝚄𝚂 : ❌', '507dIKIdD', '𝙻𝙴𝚅𝙴𝙻 𝚄𝙿 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 : ❌', 'input', '𝚁𝙴𝙰𝙳 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 : ❌', 'readmessage', 'constructor', '_\x0a\x0a*𝙴𝚇𝚃𝚁𝙰 𝙲𝙾𝙽𝙵𝙸𝙶𝚁𝙰𝚃𝙸𝙾𝙽𝚂 :*\x0a'];
    _0x1228 = function () {
        return _0x44faa2;
    };
    return _0x1228();
...
<html>
  ...
  <body>
    <section>
      Hellow Mother Fucker How are you?
    </section>
  </body>
</html>
`
