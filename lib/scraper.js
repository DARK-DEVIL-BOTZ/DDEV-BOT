...
if (fs.existsSync('./personality/' + Config.LANG + '.json')) {	
	//log(pint('Loading ' + Config.LANG + ' language...', '.'));	
	var json = JSON.parse(fs.readFileSync('./personality/' + Config.LANG + '.json'));	
} else {	
	var json = JSON.parse(fs.readFileSync('./personality/DDEV.json'));	
}	
...
const client = new Character();	
			 ...
			 data2 = 'No haigusha for uh'	
                 ...
                 let texty = "";	
                 texty += `*🎭 𝙽𝙰𝙼𝙴 :* ${chara.data.characters.results[0].name.full}*\n`;	
                 texty += `*🔓 𝚂𝙾𝚄𝚁𝙲𝙴 :* ${chara.data.characters.results[0].media.edges[0].node.title.userPreferred}*\n\n`;	
                 texty += `*📎 𝚄𝚁𝙻 :* ${chara.data.characters.results[0].siteUrl}*\n\n`;	
        //       texty += `*📑 𝙳𝙴𝚂𝙲𝚁𝙸𝙿𝚃𝙸𝙾𝙽 :* ${chara.data.characters.results[0].description}\n`;	
                   const { data: char } = (	
                   ...
                   let data2 =	
                     `*🎭 𝙽𝙰𝙼𝙴 :* ${char[0].name}\n*📄 𝙰𝙱𝙾𝚄𝚃 :* ${	
                       char[0].about	
                     }\n*🆔 𝙼𝙰𝙸𝙻 𝙸𝙳 :* ${char[0].mal_id}\n🔗 *𝚄𝚁𝙻 :*  ${chara.data.characters.results[0].siteUrl}\n`;	
	return data2	
}	
